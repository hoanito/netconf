...
def getACLfromCM(cisco_manager, AGname):
    ...
    if(ACL_isFound == False):
        print("\t%s%s--- ACL: %s DOES NOT EXIST%s"%(color.BOLD, color.FAIL, AGname, color.ENDC))



# Used to check both source or destination tag. Use either "dest-ipv4-address" or "ipv4-address" as for "tag" param
...
# Used to check both source or destination tag. Use either "src-eq" or "dst-eq" as for "tag" param
